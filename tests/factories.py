...
from accounts.models import User, PlayerProfile, HostProfile
from tournaments.models import Tournament, Scrim, TournamentRegistration, ScrimRegistration, HostRating
...
class TournamentRegistrationFactory(DjangoModelFactory):
    ...
    def in_game_details(self):
        return {
            'ign': fake.user_name(),
            'uid': f'UID{fake.random_number(digits=6)}',
            'rank': 'Gold'
        }
