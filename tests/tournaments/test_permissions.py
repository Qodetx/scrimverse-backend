...
from tests.factories import TournamentFactory, HostProfileFactory, UserFactory, TournamentRegistrationFactory
...
def test_player_can_rate_host(authenticated_client, host_user):
    ...
    response = authenticated_client.post(
        f'/api/tournaments/host/{host_user.host_profile.id}/rate/',
        data
    )
...
def test_guest_cannot_register_for_tournament(api_client, tournament):
    ...
    data = {
        ...
        'team_members': ['Guest'],
        'in_game_details': {'ign': 'Guest', 'uid': 'UID'}
    }
